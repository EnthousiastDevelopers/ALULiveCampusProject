...
public class Mu_startApplicationAsyncTask extends AsyncTask<Activity, Void, Void> {
	...
	protected Void doInBackground(Activity... arg0) {
		...
		File myfile = new File(Dir, "Faq.txt");

		if (myfile.exists()) {
			Log.d("hello", "File Exists");
			FileExists = true;
			try {
				new Mu_XMLParserClass();
	...
	protected void onPostExecute(Void result) {
		...
		if (FileExists == true) {
			...
			Intent i = new Intent();
			i.setClass(myActivity.getApplicationContext(), Mu_MainList.class);
