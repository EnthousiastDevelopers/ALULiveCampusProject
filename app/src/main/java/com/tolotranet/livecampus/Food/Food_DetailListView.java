...
import com.tolotranet.livecampus.Com.Com_ItemObject;
import com.tolotranet.livecampus.Com.Com_MainList;
import com.tolotranet.livecampus.Com.Com_MyCustomBaseAdapter;
...
public class Food_DetailListView extends Activity {

    ListView lv;
    Food_DetailListViewAdapter myPersonDetailListViewAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CommentsItemArray = comments.MakeArrayList(Food_XMLParserClass.q12.get(Index)); //Because we need to use the object of the item opened from the clicked index to create an arraylist of its answer and comments form com_mainlist makearraylist
        ...
        myPersonDetailListViewAdapter = new Food_DetailListViewAdapter(this, DetailList);
        ...
        answerBtn.setOnClickListener(new View.OnClickListener() { //button to send the answer
            ...
            public void onClick(View v) {
                ...
                if (answertxt.length()>15) { //because we dont want the user to send empty or short comments
                    ...
                    comment_parentid = Food_XMLParserClass.q12.get(Index); //because q12 is xml parser class is the object id
        ...
        VoteCount.setText(Food_XMLParserClass.q7.get(Index));// because we need to set the number of vote bigger and dynamic
        ...
        col3_s = "Voted";
        col2_s = Food_XMLParserClass.q13.get(Index); //because q13 is the author of this post that receive the votes
        col4_s = Food_XMLParserClass.q12.get(Index); //because q12 is the object unique id
        col5_s = "0"; // because we initialise it, it will be changed in when the user click the arrows
        col6_s = "Normal";

        if ((Food_XMLParserClass.q14.get(Index).equals(""))) { //because q14 should contains the list of the voters from google sheet script, "" means no one voted it before, so there is no need to find the user's voteState since it doesnt exist
            ...
            try { //because is the json parsing might be incorrect
                ...
                JSONArray arrayJson = new JSONArray(Food_XMLParserClass.q14.get(Index)); //because the json received start with [ ena with ], its an array of objects with propriety
                ...
                for (int n = 0; n < arrayJson.length(); n++) { //because the length is the number of unique user who voted this
                    ...
                    if (theobj.getString("author").equals(Sign_User_Object.Email)) {
                        ...
                        Log.d("hello", String.valueOf(scoreGiven) + Food_XMLParserClass.q14.get(Index));
    ...
    private ArrayList<Food_DetailListItem> getPersonalDetails(int Index) {
        ...
        if (!(Food_XMLParserClass.q5.get(Index).equals("") || Food_XMLParserClass.q5.get(Index).startsWith(nullTag))) {
        ...
        if (!(Food_XMLParserClass.q3.get(Index).equals("") || Food_XMLParserClass.q3.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
        ...
        if (!(Food_XMLParserClass.q6.get(Index).equals("") || Food_XMLParserClass.q6.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
        ...
        if (!(Food_XMLParserClass.q4.get(Index).equals("") || Food_XMLParserClass.q4.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
        ...
        if (!(Food_XMLParserClass.q7.get(Index).equals("") || Food_XMLParserClass.q7.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
            sr.setDetailName("Vote");
            sr.setDetailValue(Food_XMLParserClass.q7.get(Index));
            DetailList.add(sr);
        }
        if (!(Food_XMLParserClass.q8.get(Index).equals("") || Food_XMLParserClass.q8.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
        ...
        if (!(Food_XMLParserClass.q9.get(Index).equals("") || Food_XMLParserClass.q9.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
            sr.setDetailName("phonenumber1");
            sr.setDetailValue(Food_XMLParserClass.q9.get(Index));
            DetailList.add(sr);
        }
        if (!(Food_XMLParserClass.q10.get(Index).equals("") || Food_XMLParserClass.q10.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
        ...
        if (!(Food_XMLParserClass.q11.get(Index).equals("") || Food_XMLParserClass.q11.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
        ...
        if (!(Food_XMLParserClass.q12.get(Index).equals("") || Food_XMLParserClass.q12.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
            sr.setDetailName("apartment");
            sr.setDetailValue(Food_XMLParserClass.q12.get(Index));
            DetailList.add(sr);
        }
        if (!(Food_XMLParserClass.q13.get(Index).equals("") || Food_XMLParserClass.q13.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
            sr.setDetailName("residence");
            sr.setDetailValue(Food_XMLParserClass.q13.get(Index));
            DetailList.add(sr);
        }
        if (!(Food_XMLParserClass.q14.get(Index).equals("") || Food_XMLParserClass.q14.get(Index).startsWith(nullTag))) {
            sr = new Food_DetailListItem();
