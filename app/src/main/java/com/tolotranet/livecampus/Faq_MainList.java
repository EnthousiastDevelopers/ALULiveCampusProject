...
public class Faq_MainList extends AppCompatActivity {
	...
	private ArrayList<Faq_ItemObject> MakeArrayList() {
		...
		for (int i = 0; i < Faq_XMLParserClass.q1.size(); i++) {
			...
			if (!Faq_XMLParserClass.q2.get(i).equals("")) {
				...
				if ((Faq_XMLParserClass.q3.get(i).startsWith(nullTag))) {
					CIO.setBottomText("");
				} else {
					CIO.setBottomText(Faq_XMLParserClass.q3.get(i));
				}
				;
				if (Faq_XMLParserClass.q7.get(i).equals("")) { //because the votes on the database or >0 or empty, we want to show values
					CIO.setVotes(0);
				}else{
					CIO.setVotes(Double.parseDouble(Faq_XMLParserClass.q7.get(i)));
				}
				if (Faq_XMLParserClass.q6.get(i).equals("")) { //because the comments on the database or >0 or empty, we want to show values
					CIO.setComments(0);
				} else {
					CIO.setComments(Integer.parseInt(Faq_XMLParserClass.q6.get(i)));
				}
				CIO.setRightText(Faq_XMLParserClass.q5.get(i)); //because q9 is the category of the Faq, breakfast, lunch or dinner
				CIO.setIndex(i);
